...
// ** My code starts here **
const car = (inventory.filter(function (x){ 
  return x.car_year === 2011 && x.car_make === 'Jeep' && x.car_model === 'Wrangler';})
);
console.log(car);
...
let carYears = [];
// var result = inventory.filter(inv => {
//     return inv === car_year;
//   })
// console.log(result);
...
// ** My code starts here **
let oldCars = [];
for ( let i = 0; i < inventory.length; i++) { if (inventory[i].car_year < 2000) oldCars.push ( inventory[i]) }
console.log(oldCars); 
...
// ** My code starts here **
let BMWAndAudi = [];
for ( let i = 0; i < inventory.length; i++) { if (inventory[i].car_make === "Audi" || inventory[i].car_make === "BMW") BMWAndAudi.push (inventory[i]) }
